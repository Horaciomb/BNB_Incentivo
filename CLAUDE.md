...
- **Rotación de credenciales — checklist.** `bex_app`/`bex_ingeniero` se comparten con `rrhh-app`/`Lab` y están pendientes de rotación (bloqueada hasta que se defina el almacén compartido del equipo; aviso del 05-sep-2026). El día que se roten hay que tocar tres sitios: (1) el `.env` del servidor, (2) **reiniciar** `web_bnb_convocatoria` — el `.env` sólo se lee al arrancar — y (3) la variable de entorno local `RRHH_PG_PASSWORD` que consume `.mcp.json`. Si se olvida alguno **la app no se cae y `/api/health` sigue diciendo `"database": "configurada"`** (sólo comprueba que las variables no estén vacías, no que conecten): `_calcular_incentivos()` atrapa el error de conexión y sirve `FALLBACK_ROSTER`. La señal real es `es_respaldo: true` en `/api/incentivos/<id>` (banner de «datos de demostración» en la UI) y el `Error conectando a BD PostgreSQL` en `api\stderr.log`.
